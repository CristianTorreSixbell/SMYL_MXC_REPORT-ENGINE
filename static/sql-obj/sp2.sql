CREATE PROCEDURE GetConversationDetailsByDate @StartDate DATETIME,
...
fi nInteraccionAgente DATETIME,
...
WHERE
    conversationStart BETWEEN @StartDate
    AND @EndDate;
...
FETCH NEXT
FROM
    ConversationCursor INTO @conversationId,
    @conversationStart,
    @conversationEnd;

WHILE @@FETCH_STATUS = 0 BEGIN -- Obtener participantes asociados al ConversationId 
...
SELECT
    ...
    purposeId
FROM
    [insight_smnyl].[dbo].[Dim_Participant]
WHERE
...
FETCH NEXT
FROM
    ParticipantCursor INTO @participantId,
    @participantName,
    @purposeId;

WHILE @@FETCH_STATUS = 0 BEGIN -- Obtener sesiones asociadas al ParticipantId             
INSERT INTO #Result             
SELECT                 
...
(SELECT
        MIN(CASE
                WHEN SM.name_g = 'tAcd' THEN DATEADD(HOUR, -6, SM.emitDate)
            END)
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
        SM.sessionId = S.sessionId
) AS salidaCola,
(
    SELECT
        FORMAT(DATEADD(SECOND,DATEDIFF(SECOND,
                    MIN(CASE
                            WHEN SM.name_g = 'nOffered' THEN SM.emitDate
                        END),
                    MIN(
                        CASE
                            WHEN SM.name_g = 'tAcd' THE N SM.emitDate
                        END)),0),'HH:mm:ss')
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
...
(SELECT
    MIN(CASE
            WHEN SM.name_g = 'nConnected' THEN DATEADD(HOUR, -6, SM.emitDate)
        END)
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
...
(SELECT
    MIN(CASE
        WHEN SM.name_g = 'tConnected' THEN DATEADD(HOUR, -6, SM.emitDate)
        END)
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE SM.sessionId = S.sessionId
) AS finInteraccionCliente,
(SELECT FORM AT(DATEADD(SECOND,DATEDIFF(SECOND,
                    MIN(
                    CASE 
                        WHEN SM.name_g = 'nConnected' THEN SM.emitDate
                    END
                    ),
                    MIN(
                    CASE
                        WHEN SM.name_g = 'tConnected' THEN SM.emitDat e
                    END)), 0),'HH:mm:ss')
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
...
(SELECT MIN(CASE
                WHEN SM.name_g = 'tAnswered' THEN DATEADD(HOUR, -6, SM.emitDate)
            END)
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
...
(SELECT MIN(CASE
                WHEN SM.name_g = 'tAcw' THEN DATEADD(HOUR, -6, SM.emitDate)
            END)
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
    WHERE
...
(SELECT FORMAT(DATEADD(SECOND,DATEDIFF(SECOND,MIN(
                        CASE
                            WHEN SM.name_g = 'tAnswered' THEN SM.emitDate
                        END
                    ),MIN(
                        CASE
                            WHEN SM.name_g = 'tAcw' THEN SM.emitDate
                        END
                    )),0),'HH:mm:ss')
    FROM
        [dbo].[Dim_SessionMetrics] AS SM
...
FETCH NEXT FR OM ConversationCursor INTO @conversationId,
@conversationStart,
@conversationEnd;
